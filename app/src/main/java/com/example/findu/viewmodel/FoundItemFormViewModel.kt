    ...
    fun submitFoundItem(foundItem: FoundItem) {
        viewModelScope.launch {
            try {
                ...
                // 2. 反向匹配：查找所有同类别的 SEARCHING 状态的 LostItem (从云端)
                val searchingLostItems = SupabaseRepository.getSearchingLostItems(foundItem.category.name)
                ...
                for (lostItem in searchingLostItems) {
                    ...
                    if (score > 0.7) { 
                        ...
                        val notification = Notification(
                            ...
                            relatedItemId = lostItem.id // 关联到失主的 LostItem ID，方便失主点击查看
                        )
